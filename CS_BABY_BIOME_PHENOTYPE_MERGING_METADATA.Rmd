...
static_1<-read.delim("CS_Baby_Biome_DATA_2023_02_16_RC.txt") # static phenotypes downloaded from Redcap in feb 2023
static_2<-read.delim("CS_Baby_biome_phenotypes_extra_static.txt") # Extra static phenotypes derived from patient record (APGAR scores, P valued for growth)
dynamic<-read.delim("CS_BABY_BIOME_PHENOTYPES_DIET_EXPOSURES.txt") # Dynamic phenotypes chiefly feeding mode
merged<-list(static_1,static_2,dynamic) %>% reduce(full_join, by='ID_original')
#write.table(merged, "PHENOTYPES_CS_BABY_BIOME_17_02_2023.txt", row.names = T, quote = F, sep = "\t")
...
taxa_transposed$NG_ID[grepl("D4526W01F59G6",taxa_transposed$NG_ID)]<-"D4526MOMF59G6" 
taxa_transposed$NG_ID[grepl("D26_2M0662D4_",taxa_transposed$NG_ID)]<-"D4526M0662D4" 
...
phenotypes<-read.delim("~/Desktop/CS Baby Biome/PHENOTYPES/PHENOTYPES_CS_BABY_BIOME_17_02_2023.txt")
phenotypes$CS_BABY_BIOME_ID=NULL
all<- left_join(metadata, phenotypes)
write.table(all, "METADATA_CS_BABY_BIOME_22_02_2023.txt", row.names = F, quote = F, sep = "\t")
```
